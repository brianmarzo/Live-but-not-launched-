export const FRAMEWORK = {
  ...
  gates: [
    ...
    },
    {
      ...
      summary: "Check any that apply. If any risk is present, the case cannot be launched — resolve it first.",
      riskMode: true,
      items: [
        {
          id: "g2-1",
          title: "There are open support tickets on this account",
          detail: "Active complaints, critical bugs, escalations, or any unresolved issue that hasn't been closed — including tickets opened by the customer or flagged internally.",
        },
        {
          id: "g2-2",
          title: "There is a churn or cancellation signal",
          detail: "Customer has expressed a desire to cancel, been unresponsive since going live, flagged dissatisfaction with the product, or is at risk of churning within 30 days.",
        },
        {
          id: "g2-3",
          title: "Customer is experiencing post-launch instability",
          detail: "Active reports of site outages, menu errors, ordering failures, or integration problems — from the customer, their guests, or flagged internally.",
        },
      ],
      fail: {
        kind: "stop",
        title: "Do not launch.",
        body: "There is a real risk signal on this account. Closing the case while an issue is open or churn risk exists is not in the company's interest. Resolve the issue first, then revisit.",
      },
      pass: {
        title: "No risk signals found.",
        body: "No open tickets, no churn signal, no instability. The company has no reason to hold this case open. Move to Gate 3.",
      },
    },
    {
      id: "g3",
...
export function deriveOutcome(state) {
  ...
  if (state.gate2.some(Boolean)) return { status: "blocked", on: "g2", outcome: "hold" };
