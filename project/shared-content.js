// Shared content & schema for the GP Score Launch Decision Framework
// Single source of truth — all three variations consume this.
...
window.FRAMEWORK = {
  ...
  gates: [
...

    {
      id: "g2",
      n: "02",
      title: "Is there a churn or support risk?",
      kicker: "Company",
      kickerColor: "deep-blue",
      summary: "If any of these are true, do not launch — there is a real company risk to protect. If all are clear, the company has no reason to hold this case.",
      items: [
        {
          id: "g2-1",
          title: "No open support tickets",
          detail: "There are no active support issues on this account. No complaints, no critical bugs, no escalations.",
        },
        {
          id: "g2-2",
          title: "No churn signal or cancellation conversation",
          detail: "The customer has not expressed a desire to cancel, has not been unresponsive after going live, and is not at risk of 30-day churn.",
        },
        {
          id: "g2-3",
          title: "Customer is stable post-launch",
          detail: "The customer is operating normally. No reports of site issues, menu errors, or ordering problems from their team or their guests.",
        },
      ],
      fail: {
        kind: "stop",
        title: "Do not launch.",
        body: "There is a real risk signal on this account. Closing the case while the customer has an open issue or is at churn risk is not in the company's interest. Resolve the issue first.",
      },
      pass: {
        title: "No company risk found.",
        body: "No support tickets, no churn signal, customer is stable. Holding this case open now only benefits one person. Move to Gate 3.",
      },
    },
...
// Derive an outcome from current state.
// state: { gate1: bool[], gate2: bool[], branch: 'uncontrollable'|'well-configured'|'real-blocker'|null, gate4: bool[] }
window.deriveOutcome = function (state) {
  ...
  if (!all(state.gate2, 3)) return { status: "blocked", on: "g2", outcome: "hold" };
